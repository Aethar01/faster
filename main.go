...
func renderRow(currentSpeed float64, speeds []float64, peak float64, ping time.Duration, sparkStyle lipgloss.Style) string {
	...
	if ping > 0 {
		s.WriteString(unitStyle.Render(fmt.Sprintf("  ping %dms", ping.Milliseconds())))
	}
	return s.String()
}
...
func (m Model) View() string {
	...
	var s strings.Builder
	s.WriteString(renderRow(m.dlSpeed, m.dlSpeeds, m.dlPeak, m.ping, dlSparkStyle))
	if m.dlDone {
		s.WriteString("\n")
		s.WriteString(renderRow(m.ulSpeed, m.ulSpeeds, m.ulPeak, 0, ulSparkStyle))
	}
